...
{
    SetConsoleTitleW(L"InjectDllUtil v.1.0");
    printf("InjectDllUtil v.1.0 通用 DLL 注入工具；\n");
    if (argc != 3)
    {
    ...
    HMODULE mod = LoadLibraryA(R"(S:\CLionProjects\attachDll\cmake-build-debug-visual-studio\attach1Dll.dll)");
    std::cout << mod << std::endl;
    func proc = (func)::GetProcAddress(mod, "getDllPoint");

    HOOKPROC fp = proc();
    HHOOK hhk = ::SetWindowsHookEx(WH_MOUSE, fp, mod, thread_id);*/
    return 0;
}
